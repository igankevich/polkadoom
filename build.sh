...
mkdir -p output
rm -f output/doom$TARGET_BITNESS.elf
...
build() {
    outfile=output/doom$TARGET_BITNESS-$1.elf
    clang $TARGET_FLAGS \
        -Wl,--error-limit=0 \
        -Wl,--emit-relocs \
        -Wl,--no-relax \
        ...
        $DOOM_ROOT/mus2mid.c \
        -o "$outfile"
    ../polkavm/target/release/polkatool link -s "$outfile" -o "$outfile".polkavm
}
...
build_guest() {
    ...
    cargo test --package jam-node --lib -- --nocapture core_vm_doom
}
...
build_builder() {
    ...
    polkadoom="$PWD"
    cd ../polkajam
    cargo run --package doom-builder -- \
        --builder "$polkadoom"/output/doom64-builder.elf.polkavm \
        --guest "$polkadoom"/output/doom64-guest.elf.polkavm
    #cd ../polkavm/examples/doom
